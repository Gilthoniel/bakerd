...
impl Interceptor for Authorization {
  fn call(&mut self, mut req: Request<()>) -> std::result::Result<Request<()>, Status> {
    req.metadata_mut().insert("authentication", self.token.clone());
...
mod tests {
  ...
  async fn test_get_node_uptime() {
    ...
    service.expect_peer_uptime().times(1).returning(|_| {
      Ok(Response::new(ccd::NumberResponse {
        value: 42,
      }))
    });
  ...
  async fn test_get_last_block() {
    ...
    service.expect_get_consensus_status().times(1).returning(|_| {
  ...
  async fn test_get_account_info_bad_json() {
    ...
    service.expect_get_account_info().times(1).returning(move |_| {
      Ok(Response::new(ccd::JsonResponse {
